...
mynormalmixEM <- function(x, maxit, tol){
  n <- length(x)
  w1 <- double(n)
  w2 <- double(n)
  xi1 <- .5
  ...
  for (i in 1:maxit){
    ...
    sigma1new <- sum(w1 * (x - mu1new)^2) / sum(w1)
    sigma2new <- sum(w2 * (x - mu2new)^2) / sum(w2)
...
mynormalmixEM(test$Response, 1e5, 1e-4)  
